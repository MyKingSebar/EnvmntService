...
public class EnvMntManager {
	...
	private boolean initSerialPort(){
		try {
			mSerialPort = new SerialPort(new File(DEVFILE_SERIALPORT), BAUTRATE, 8, 1, 'n');
	...
	public void updateMonitorDevice(String cpuId, String mac, String term, String termGrp) {
		mCpuId = cpuId;
		mMac = mac;
		
		MonitorDevice device = new MonitorDevice(cpuId, mac);
		device.setName(term);
		device.setGroup(termGrp);
	...
	private void sendReadCmd(int type) {
		...
		case EnvMntConstants.READTYPE_GPSRECVDATA:
			...
			break;
		default:
			Logger.i("Unknown read type, type = " + type + ".");
			return;
		}
	...
	private boolean isDataValid(int type) {
		...
		default:
			Logger.i("Invalid read type, type = " + type + ".");
			return false;
		}
	}
	...
	private void sendMonitorCmds() {
		...
		if (!EnvMntParamManager.getInstance().isSensorSpcValid()) {
			Logger.i("Failed to read sensor space info.");
	...
	private void submitAirConStats() {
		if (!EnvMntParamManager.getInstance().isAirConStatsValid()) {
			Logger.i("Air conditioner status info is invalid.");
			return;
		}
	...
	private void submitDrtFanStats() {
		if (!EnvMntParamManager.getInstance().isDrtFanStatsValid()) {
			Logger.i("Draft fan status info is invalid.");
			return;
		}
	...
	private void submitOtherStats() {
		if (EnvMntParamManager.getInstance().isLcdDrvBoardValid()) {
			...
			Logger.i("Lcd driver board info is invalid.");
		}
		...
		if (EnvMntParamManager.getInstance().isGpsRecvDataValid()) {
			...
			Logger.i("GPS received data info is invalid.");
		}
		...
		if (!islgtradsensorvalid && !ispm25sensorvalid && !issensor1valid && !issensor2valid) {
			Logger.i("Light radiation sensor info, pm 2.5 sensor info, sensor1 and sensor2 all are invalid.");
			return;
		}
	...
	private final class MonitorThread extends Thread {
		...
		public void cancel() {
        	Logger.i("Cancel the monitor thread.");
        	mIsCanceled = true;
            interrupt();
        }
		...
		@Override
		public void run() {
			Logger.i("A new monitor thread is started. Thread id is " + getId() + ".");
            ...
            Logger.i("Monitor thread is safely terminated, id is: " + currentThread().getId());
		}
	}
	...
	private void handleLcdDrvBoard(byte[] data) {
		...
		if (info.start_flag != 0xFF) {
			Logger.i("The start flag of LCD driver board is invalid, flag = " + info.start_flag + ".");
			return;
		}
	...
	private void handleLgtRadSensor(byte[] data) {
		...
		if (info.address != 0x01) {
			Logger.i("The address domain of light radiation sensor is invalid, addr = "
					+ info.address + ".");
			return;
		}
		...
		if (info.function != 0x85) {
			Logger.i("The function domain of light radiation sensor is invalid, func = "
					+ info.function + ".");
			return;
		}
		...
		if (info.datalength != 0x0004) {
			Logger.i("The data domain length of light radiation sensor is invalid, len = "
					+ info.datalength + ".");
			return;
		}
	...
	private void handlePm25Sensor(byte[] data) {
		...
		if (info.start_flag != 0xAA) {
			Logger.i("The start flag of PM2.5 sensor is invalid, flag = " + info.start_flag + ".");
			return;
		}

		info.end_flag = data[6] & 0xFF;
		if (info.end_flag != 0xFF) {
			Logger.i("The stop flag of PM2.5 sensor is invalid, flag = " + info.end_flag + ".");
			return;
		}
		...
		if (calc_checksum != info.checksum) {
			Logger.i("The checksum of PM2.5 sensor is incorrect, calculated checksum = "
	...
	private void handleGpsRecvData(byte[] data) {
		...
		if (gpgga_start_pos != -1) {
			...
			if (gpgga_end_pos != -1) {
				...
				if (gpgga_array == null) {
					Logger.i("GPGGA array is null.");
					return;
				} else if (gpgga_array.length != 15) {
					Logger.i("The length of GPGGA is invalid, len = " + gpgga_array.length + ".");
					return;
					...
					Logger.i("Invalid GPS data: " + gpgga_str);
					return;
				}
				...
				Logger.i("Can't find the end flag of the GPGGA data.");
			}
		} else {
			Logger.i("Can't find \"$GPGGA\" in the gps data.");
		}
	}
	...
	private void onDataReceived(byte[] buffer, int size) {
		if (mPacketLen == 0) {
			if ((size == 1) && (buffer[0] == EnvMntConstants.CMDRESP_CFGSPC_WRITE)) {
				Logger.e("Receive the writing response ("
						+ EnvMntConstants.CMDRESP_CFGSPC_WRITE + ").");
				return;
			} else if ((buffer[0] != 'B') || (buffer[1] != 'J')
					|| (buffer[2] != 'Y') || (buffer[3] != 'S')) {
				Logger.e("Received data is invalid, data[0] = " + buffer[0]
			...
			if (buffer[4] != mCurrentReadType) {
				Logger.e("The type of received data is incorrect, ptype = " + buffer[4]
			...
			if (len > (EnvMntConstants.MAX_READ_LEN - EnvMntConstants.READ_HEAD_LEN)) {
				Logger.e("The current total length of received data is invalid, len = " + len + ".");
	...
	private class ReadThread extends Thread {
		...
		public void cancel() {
        	Logger.i("Cancel the read thread.");
        	mIsCanceled = true;
        }

		@Override
		public void run() {
			Logger.i("A new read thread is started. Thread id is " + getId() + ".");
			while (!mIsCanceled) {
				try {
					if (mInputStream == null) {
						Logger.i("Input stream is null.");
						return;
					}
					mRecvSize = mInputStream.read(mRecvBuf);
					if ((mRecvSize <= 0) || (mRecvSize > EnvMntConstants.MAX_READ_LEN)) {
						Logger.i("Received size is invalid, size = " + mRecvSize + ".");
						continue;
					}
			...
			Logger.i("Read thread is safely terminated, id is: " + currentThread().getId());
		}
	}
	...
	private void doSendSerialData(byte[] data) {
		if (data == null) {
			...
			Logger.i("Data length is invalid, len = " + data.length + ".");
			return;
		}

		if (mOutputStream == null) {
			Logger.i("Output stream is null.");
			return;
		}

		try {
			Logger.i("data.length"+data.length+"/t dataStr"+data);
	...
	private class MyHandler extends Handler {
		...
		public void handleMessage(Message msg) {
            ...
            case EVENT_SEND_SERIAL_DATA:
            	...
            	break;
            default:
            	Logger.i("Unknown message, msg.what = " + msg.what);
